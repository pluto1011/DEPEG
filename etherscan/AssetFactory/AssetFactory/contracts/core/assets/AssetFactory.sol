...
contract AssetFactory is IAssetFactory, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _generateVariant(
        string memory baseSymbol,
        Id id
    ) internal returns (string memory variant) {
        ...
        uint256 variantUint = variantIndexPair[id] == 0
            ? ++variantIndex[hash]
            : variantIndexPair[id];
    ...
    function _generateSymbolWithVariant(
        ...
        Id id
    ) internal returns (string memory symbol) {
        string memory baseSymbol = IERC20Metadata(pa).symbol();
        string memory separator = expiry == 0
            ? "!"
            : Strings.toString(BokkyPooBahsDateTimeLibrary.getMonth(expiry));
    ...
    function getLv(
        ...
        return
            lvs[
                Pair(_pa, _ra, initialArp, expiryInterval, exchangeRateProvider)
                    .toId()
            ];
    }
    ...
    {
        SwapPair[] storage _assets = swapAssets[
            Pair(_pa, _ra, _initialArp, _expiryInterval, _exchangeRateProvider)
                .toId()
        ];
    ...
    function deploySwapAssets(
        ...
        Pair memory asset = Pair(
            params._pa,
            params._ra,
            params.initialArp,
            params.expiryInterval,
            params.exchangeRateProvider
        );
        Id id = asset.toId();
    ...
    function deployLv(
        ...
        ); // 이걸 가스비를 절감 안하고 따로 빼? ㄹㅈㄷ

        {
            // 요소들을 가지고 pairname을 만드는 거 같음. 이거 겹치면 어떻게 되냐? create1으로 만드는 거는 보이긴 하는데.
            string memory pairname = _generateSymbolWithVariant(
                _pa,
                0,
                LV_PREFIX,
                pair.toId()
