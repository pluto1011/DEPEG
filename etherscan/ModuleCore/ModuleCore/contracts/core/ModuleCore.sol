    ...
    VaultCore
{
    ...
    ) external pure returns (Id) {
        return
            PairLibrary
                .initalize(pa, ra, initialArp, expiry, exchangeRateProvider)
                .toId();
    } // initialize를 왜붙였냐고요! 깜짝 놀랐잖아 ㅜㅜ
    ...
    function initializeModuleCore(
        ...
        Pair memory key = PairLibrary.initalize(
            pa,
            ra,
            initialArp,
            expiryInterval,
    ...
    function _initOnNewIssuance(
        ...
        emit Issued(
            id,
            idx,
            block.timestamp + _expiryInterval,
            ds,
            ct,
            AmmId.unwrap(toAmmId(ra, ct))
        );
    }

    function updateRepurchaseFeeRate(
        Id id,
        uint256 newRepurchaseFeePercentage
    ) external {
        onlyConfig();
        State storage state = states[id];
        PsmLibrary.updateRepurchaseFeePercentage(
            state,
            newRepurchaseFeePercentage
        );
     ...
     */
    function updatePsmBaseRedemptionFeePercentage(
        Id id,
        uint256 newPsmBaseRedemptionFeePercentage
    ) external {
        onlyConfig();
        State storage state = states[id];
        PsmLibrary.updatePSMBaseRedemptionFeePercentage(
            state,
            newPsmBaseRedemptionFeePercentage
        );
        emit PsmBaseRedemptionFeePercentageUpdated(
            id,
            newPsmBaseRedemptionFeePercentage
        );
    }
